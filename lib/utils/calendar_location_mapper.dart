import 'package:bugaoshan/utils/calendar_event_models.dart';

/// 上课地点 → 校区/建筑/房间 的解析器。
///
/// 从 `calendar_event_utils.dart` 拆分而来：此处只保留「位置解析 + 静态地理
/// 参考数据」，位置模型类见 `calendar_event_models.dart`。
///
/// 匹配策略（详见 [CalendarLocationMapper.resolve]）：
/// 1. 从 campusName / location 识别目标校区；
/// 2. 匹配高精度建筑并以「最长 pattern 优先」消除歧义；
/// 3. 回退到校区级推断，最后回退到原样返回。
